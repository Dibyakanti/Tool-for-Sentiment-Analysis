const HOSTED_URLS = {
    queryTwitter:  window.location.protocol + '//'+ window.location.hostname + '/js/sentiment/queryTwitter.php',
...
let urls, model, metadata,saved_tweets;
...
function twitterSentiment(){
    ...
    // getTwitterHashTagData(processTwitterData);
    // getTwitterHashTagData($("#tag-input").val(), processTwitterData);
 //    $.getJSON("./saved_tweets.json", function(saved_tweets) {
 //    console.log(saved_tweets); // this will show the info it in firebug console
	// });	
	var saved_tweet = JSON.parse(data);
    processTwitterData(saved_tweet);
}
    ...
    setupSentimentModel().then(
        result => {
            ...
            $.each(tweets, function( index, tweet ) {
                ...
                const tweet_text = tweet.replace(/(?:https?|ftp):\/\/[\n\S]+/g, '');
